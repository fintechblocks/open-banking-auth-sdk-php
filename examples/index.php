<?php
    ...
    include_once(".".DIRECTORY_SEPARATOR."src".DIRECTORY_SEPARATOR."Config.php"); // Configuration
    $modeArray["accounts"]["client_id"] = Config::$accountInfoClientId;
    $modeArray["accounts"]["base_url"] = Config::$accountInfoBaseUrl;
    $modeArray["accounts"]["scope"] = Config::$accountInfoScope;

    //Example "mode" parameters -- payments
    $modeArray["payments"]["client_id"] = Config::$paymentInitClientId;
    $modeArray["payments"]["base_url"] = Config::$paymentInitBaseUrl;
    $modeArray["payments"]["scope"] = Config::$paymentInitScope;
    ...
    if (isset($_GET["example_function"]) || isset($_SESSION["example_function"])){
        ...
        include_once(__DIR__.DIRECTORY_SEPARATOR."src".DIRECTORY_SEPARATOR."Functions.php"); // A collection of API functions
        include_once(__DIR__.DIRECTORY_SEPARATOR."src".DIRECTORY_SEPARATOR."Connect.php"); // A simple class to create the cUrl calls. You can use your own as well.
        ...
        if (!isset($_GET["code"]) && (!isset($_SESSION["loggedIn"]) || !$_SESSION["loggedIn"])) {
            ...
            if ($_SESSION["example_function"] == "accounts") {
                ...
                $requestBody->Data->ExpirationDateTime = "2031-08-02T00:00:00+00:00";
                ...
                $issuer = "C=UK, ST=England, L=London, O=Acme Ltd.";
                $signatureHeader = $example->createSignatureHeader($requestBody, $_SESSION["kid"], $issuer);
                $intentId = $functions->createDomesticPaymentConsentRequest($requestBody, $accessToken,["Content-Type: application/json","x-idempotency-key: ".$xIdempotencyKey, "x-jws-signature:".$signatureHeader]);
        ...
        if (isset($_GET["code"]) && (!isset($_SESSION["loggedIn"]) || !$_SESSION["loggedIn"])){
            if (!isset($_SESSION["user_access_token"]) || $_SESSION["user_access_token"] == "") {
                ...
                $accessAndRefreshToken = $example->tokenExcange($_GET["code"]);
                $userAccessToken = $accessAndRefreshToken->access_token;
